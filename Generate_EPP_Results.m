...
% load specifications
ACSpecs = AircraftSpecsPkg.ATR42_FuelCell;
ACSpecs.Settings.Plotting = 0;
...
for ii = 1:N
    for jj = 1:N
        ACSpecs = AircraftSpecsPkg.ATR42_Elec(EtaGrid(ii,jj));
        ACSpecs.Settings.Plotting = 0;
        ACSpecs.Specs.Power.SpecEnergy.Batt = eBattGrid(ii,jj);
        SizedAC(ii,jj) = Main(ACSpecs,@MissionProfilesPkg.ATR42_600);
        EnergyGrid(ii,jj) = SizedAC(ii,jj).Mission.History.SI.Energy.E_ES(end);
...
Specs = AircraftSpecsPkg.A320_FuelCell();
...
save('EPP_Results/BRE_ATR_FuelCell.mat','Sized','RangeGrid','MBM_BRE','MBM_BRE_Norsv','MBM_FAST')
...
save('EPP_Results/BRE_ATR_Elec.mat','Sized','RangeGrid','MBM_BRE','MBM_BRE_Norsv','MBM_FAST')
